...
async fn test_large_file_uploads_and_dedup() {
    ...
    let storage_service = Arc::new(StorageService::new(s3_client.clone(), "uploads".to_string()));
    ...
    let app = create_app(state)
        .layer(axum::extract::DefaultBodyLimit::max(sec_config.max_file_size));
    ...
    let response = app.clone()
        .oneshot(
            Request::builder()
                ...
                .body(Body::from(r#"{"username": "bigfileuser", "password": "password123"}"#))
                .unwrap(),
        )
        .await
        .unwrap();
    ...
    let upload_file = |size_mb: usize, filename: &str| {
        ...
        async move {
            ...
            println!("Uploading {}MB file (payload size: {} bytes)...", size_mb, len);
            ...
            (response.status(), response.into_body().collect().await.unwrap().to_bytes())
        }
    };
    ...
    assert_eq!(key_50, key_50_copy, "Both 50MB user files should point to same storage file");

    // B. Verify MinIO Existence
    // We check if the keys exist in the bucket.
    
    // 50MB File
    assert!(storage_service.file_exists(&key_50).await.unwrap(), "50MB file validation in S3 failed");
    ...
    .await
    .unwrap();
    assert!(storage_service.file_exists(&key_200).await.unwrap(), "200MB file validation in S3 failed");
    ...
    .await
    .unwrap();
    assert!(storage_service.file_exists(&key_300).await.unwrap(), "300MB file validation in S3 failed");
