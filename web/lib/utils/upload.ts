export async function scanEntries(items: DataTransferItemList | null): Promise<{ file: File, path: string }[]> {
    if (!items) return [];
        ...
        if (entry.isFile) {
            ...
            const readAllEntries = async () => {
                ...
                if (allEntries.length > 0) {
                    const promises = allEntries.map(subEntry => traverseEntry(subEntry, currentPath));
...
export async function scanFiles(files: FileList | null): Promise<{ file: File, path: string }[]> {
    if (!files) return [];
