...
export const getColumns = () => [
  columnHelper.accessor("name", {
    ...
    cell: (info) => {
      const isPending =
        info.row.original.scanStatus === "pending" ||
        info.row.original.scanStatus === "scanning";
        ...
        return (
          <div className="rfm-workspace-list-icon-td">
            <FileIcon
              ...
              isLocked={info.row.original.isLocked}
              permission={info.row.original.permission}
              hideName={true}
              className="rfm-list-icon opacity-70"
            />
            <p className="line-through opacity-70 mr-2 line-clamp-2 break-all flex-1">
              {info.getValue()}
            </p>
          ...
          {isPending && (
            <span className="rfm-scanning-badge">
              {isScanning && (
                <SvgIcon
                  svgType="cog"
                  className="rfm-spinner-small animate-spin inline-block mr-1"
  ...
  columnHelper.accessor("size", {
    header: () => "Size",
    cell: (info) =>
      info.row.original.isDir ? "--" : formatSize(info.getValue() || 0),
  }),
  columnHelper.accessor("mimeType", {
    header: () => "Type",
    cell: (info) =>
      info.row.original.isDir ? "Folder" : formatMimeType(info.getValue()),
  }),
