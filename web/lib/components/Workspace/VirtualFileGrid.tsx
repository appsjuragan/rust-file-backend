...
export const VirtualFileGrid: React.FC<VirtualFileGridProps> = ({
    ...
    const handleKeyDown = (e: React.KeyboardEvent) => {
        ...
        if (newIndex !== focusedIndex && newIndex >= 0) {
            ...
            // Scroll into view
            if (gridRef.current) {
                gridRef.current.scrollToRow({ index: Math.floor(newIndex / cols), align: 'smart' });
            }
        }
    };
...
// Cell Component
const Cell = memo(({ columnIndex, rowIndex, style, files, columnCount, handlers, ariaAttributes }: CustomCellProps & {
    ariaAttributes: any;
    columnIndex: number;
    rowIndex: number;
        ...
        <div style={adjustedStyle} {...ariaAttributes} aria-selected={isSelected}>
            <button
                ...
                className={`rfm-file-item h-full w-full 
                    ...
                    ${isFocused ? "ring-2 ring-blue-500 ring-offset-2 dark:ring-offset-gray-900" : ""}
                ...
                aria-label={f.name}
            >
                <FileIcon id={f.id} name={f.name} isDir={f.isDir} scanStatus={f.scanStatus} />
                {isPending && (
