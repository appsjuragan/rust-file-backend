...
const UploadProgressToast = () => {
    const { activeUploads, setActiveUploads, onCancelUpload, resetSignal } = useFileManager();
    ...
    const uploading = activeUploads.filter(u => u.status === 'uploading' || u.status === 'hashing' || u.status === 'processing' || u.status === 'queued').length;
    ...
    const averageProgress = total > 0
        ? Math.round(activeUploads.reduce((acc, u) => acc + (u.progress || 0), 0) / total)
        : 0;
    ...
    return (
        <div className={`rfm-upload-toast ${isMinimized ? "rfm-upload-toast--minimized" : ""}`} style={{ width: '400px', border: `2px solid ${isFinished ? borderColor : '#6366f1'}`, borderRadius: '16px' }}>
            <div className="rfm-upload-toast-header" style={{ padding: '20px 24px', display: 'flex', alignItems: 'center', justifyContent: 'between', background: 'transparent', border: 'none' }}>
                <div className="flex flex-col gap-1">
                    <span className="rfm-upload-toast-title" style={{ fontSize: '16px', fontWeight: '600' }}>
                        {uploading > 0 ? `Uploading ${total} items` : `Finished ${total} items`}
                    </span>
                    <span className="text-[12px] opacity-70">
                        {completed} of {total} completed {uploading > 0 ? `(${averageProgress}%)` : ''} {errors > 0 ? `, ${errors} failed` : ''}
                    </span>
                </div>
                <div className="rfm-upload-toast-actions" style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <button
                        onClick={() => setIsMinimized(!isMinimized)}
                        className="p-1 hover:bg-gray-700 rounded transition-colors"
                        title={isMinimized ? "Expand" : "Minimize"}
                    >
                        <SvgIcon svgType={isMinimized ? "square" : "minus"} className="w-4 h-4" />
                    </button>
                    {showCountdown && countdown !== null ? (
                        ...
                        >
                            ...
                            <svg className="absolute top-0 left-0" width="40" height="40" style={{ transform: 'rotate(-90deg)' }}>
                                <circle
                                    cx="20"
                                    cy="20"
                                    r="16"
                        ...
                        <button
                            ...
                            title="Clear all"
                        >
                            <SvgIcon svgType="close" className="w-5 h-5 text-red-400 !fill-current" />
                        </button>
                    ) : null}
                </div>
            </div>
            {!isMinimized && (
                <div className="rfm-upload-toast-body" style={{ padding: '0 24px 20px 24px' }}>
                    <div className="rfm-upload-list" style={{ maxHeight: '350px', overflowY: 'auto' }}>
                        {activeUploads.map((u) => (
                            <div key={u.id} className="rfm-upload-item" style={{ border: 'none', background: 'transparent', padding: '12px 0', alignItems: 'flex-start' }}>
                                        ...
                                        <div className="flex justify-between items-center gap-2">
                                            ...
                                            {u.status === 'hashing' && (
                                                <div className="flex items-center text-indigo-400">
                                                    <span className="text-[8px] font-bold lowercase tracking-wider">hashing...</span>
                                                </div>
                                            )}
                                            {u.status === 'processing' && (
                                    ...
                                    <div className="flex items-center gap-1 ml-auto">
                                        {u.status !== 'completed' && (
                                            <button
                                                ...
                                                title="Cancel"
                                            >
                                                <SvgIcon svgType="close" className="w-3.5 h-3.5 !fill-current" />
                                            </button>
                                        )}
                                    </div>
                                    {u.status === 'uploading' && (
