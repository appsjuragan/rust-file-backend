...
const FileIcon = (props: IFileIcon) => {
  ...
  const colorCategory = useMemo(() => {
    ...
    if (['mp4', 'mkv', 'avi', 'mov', 'wmv', 'flv', 'webm', 'mpg', 'mpeg'].includes(ext)) return "video";
    ...
    if (['txt', 'md', 'log', 'ini', 'conf', 'json', 'yml', 'yaml'].includes(ext)) return "text";
    if (['mp3', 'wav', 'ogg', 'm4a', 'flac', 'aac'].includes(ext)) return "audio";
    if (['js', 'ts', 'tsx', 'jsx', 'py', 'rb', 'php', 'c', 'cpp', 'h', 'java', 'go', 'rs', 'html', 'css'].includes(ext)) return "code";
