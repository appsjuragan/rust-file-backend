...
interface ISvgIconProps extends Omit<React.AllHTMLAttributes<HTMLDivElement>, 'size'> {
...
const SvgIcon: React.FC<ISvgIconProps> = ({
  ...
  const hasSizingClass = props.className?.split(/\s+/).some(c => c.startsWith('w-') || c.startsWith('h-'));
  const sizeStyle = size ? { width: size, height: size, minWidth: size, minHeight: size } : {};
  ...
  return (
    <div
      ...
      className={`flex items-center justify-center flex-shrink-0 overflow-hidden ${(!size && !hasSizingClass) ? "w-[1em] h-[1em]" : ""} ${props.className || ""}`}
    >
      {svgContent()}
    </div>
  );
