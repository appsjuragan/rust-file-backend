...
export const useFileActions = () => {
    ...
    const handleCopy = useCallback((targetIds?: string[]) => {
    ...
    }, [selectedIds, currentFolder, setClipboardIds, setIsCut, setClipboardSourceFolder, setSelectedIds]);

    const handleCut = useCallback((targetIds?: string[]) => {
        const ids = targetIds && targetIds.length > 0 ? targetIds : selectedIds;
        if (ids.length === 0) return;
    ...
    }, [clipboardIds, isCut, currentFolder, onBulkMove, onMove, onBulkCopy, setClipboardIds, setIsCut, setClipboardSourceFolder, onRefresh]);

    const handleDelete = useCallback((targetIds?: string[], targetName?: string) => {
        const ids = targetIds && targetIds.length > 0 ? targetIds : selectedIds;
        if (ids.length === 0) return;
    ...
    }, [selectedIds, currentFolder, setDialogState, onBulkDelete, onDelete, setSelectedIds, onRefresh]);
