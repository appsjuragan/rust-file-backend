...
export const useFileUpload = (
  ...
  useEffect(() => {
    ...
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      e.preventDefault();
      e.returnValue =
        "You have uploads in progress. If you leave, they will be lost.";
  ...
  const calculateHash = useCallback(
    async (file: File, onProgress?: (p: number) => void): Promise<string> => {
  ...
  const onUpload = useCallback(
    async (files: { file: File; path: string }[], folderId: string) => {
      ...
      const uploadQueue = files.map((f, i) => ({
        ...f,
        id: statusItems[i]!.id,
      }));
      const CONCURRENCY = 3;
        ...
        while (uploadQueue.length > 0) {
          ...
          try {
            ...
            const existing = currentFilesRef.current.find((f) => {
              const normFParent =
                !f.parentId || f.parentId === "0" || f.parentId === "root"
                  ? "0"
                  : f.parentId;
