 ...
 */
export async function storeFileForUpload(uploadId: string, file: File, meta: UploadMeta): Promise<void> {
 ...
 */
export async function getChunkData(uploadId: string, partNumber: number): Promise<ArrayBuffer | null> {
    const db = await openDb();
 ...
 */
export async function getAllUploadMeta(): Promise<{ id: string; meta: UploadMeta }[]> {
    const db = await openDb();
    const tx = db.transaction(META_STORE, 'readonly');
 ...
 */
export async function cleanupUpload(uploadId: string, totalChunks: number): Promise<void> {
    const db = await openDb();
