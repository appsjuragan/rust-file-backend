...
namespace AppsJuragan.SyncClient.ViewModels
{
    public partial class SyncStatusViewModel : ObservableObject
    {
        ...
        [ObservableProperty] 
        [NotifyPropertyChangedFor(nameof(FullAvatarUrl))]
        private UserProfileResponse? _profile;

        [ObservableProperty] private UserFactsResponse? _facts;
        [ObservableProperty] private string _storageFormatted = "0 B used";

        public string? FullAvatarUrl => Profile?.AvatarUrl != null 
            ? (Profile.AvatarUrl.StartsWith("http") ? Profile.AvatarUrl : $"{_api.BaseUrl.TrimEnd('/')}{Profile.AvatarUrl}") 
            : null;
